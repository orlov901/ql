...
module Internal {
   ...
   * `p` belongs, and `ret` having abstract value `retVal` allows the conclusion
   * that the parameter `p` either is `null` or non-`null` as specified by `isNull`.
   */
  private predicate validReturnInCustomNullGuard(Expr ret, Parameter p, BooleanValue retVal, boolean isNull) {
    exists(Callable c |
      canReturn(c, ret) |
      p.getCallable() = c and
   ...
   * `null`-check on parameter `p`. A return value having abstract value `retVal`
   * allows us to conclude that the argument either is `null` or non-`null` as
   * specified by `isNull`.
   */
  private Callable customNullGuard(Parameter p, BooleanValue retVal, boolean isNull) {
  ...
  private cached module Cached {
    ...
    predicate isCustomNullCheck(Call call, Expr arg, BooleanValue v, boolean isNull) {
      exists(Callable callable, Parameter p |
        ...
        callable = customNullGuard(p, v, isNull)
      )
    }
